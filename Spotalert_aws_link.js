...
export async function detectFaces(bucket, key) {
  ...
  // Use S3Object reference (no local file dependency)
...
// === TEMP SIGNUP ENDPOINT (for frontend test) ===
app.post("/signup", (req, res) => {
  const { name, email, password } = req.body || {};
  console.log(`🆕 Signup attempt: ${name} <${email}>`);
  return res.json({ success: true, message: "Signup route connected successfully!" });
});
